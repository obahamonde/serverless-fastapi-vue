...
class APIClient:
    ...
    async def post(self, url: str, data: Dict[str, Any]) -> Dict[str, Any]:
        async with ClientSession(headers=self.headers) as client_session:
            async with client_session.post(f"{self.base_url}{url}", json=data) as response:
                return await response.json()
    ...
    async def delete(self, url: str, params: Dict[str, str] = None) -> Dict[str, Any]:
        async with ClientSession(headers=self.headers) as client_session:
            async with client_session.delete(f"{self.base_url}{url}", params=params) as response:
                return await response.json()

    async def html(self, url: str) -> str:
        async with ClientSession(headers=HEADERS) as client_session:
            async with client_session.get(f"{self.base_url}{url}") as response:
...
class App(FastAPI):
    def __init__(self):
        ...
        @self.get("/api/auth")
        async def auth_endpoint(req: Request):
            return await self.auth(req)
        ...
        @self.get("/github/{q}")
        async def github(q:str, sort:str="stars", order:str="desc", page:int=1, per_page:int=10)->Dict[str,Any]:
            response = await self.http.get(f"/search/repositories?q={q}&sort={sort}&order={order}&page={page}&per_page={per_page}")
            return [{
                ...
                "languages": await self.http.get(f"/repos/{response['items'][i]['full_name']}/languages"),
    ...
    async def auth(self,request: Request):
        ...
        async with ClientSession(headers=headers) as client_session:
            async with client_session.get(f"https://{AUTH0_DOMAIN}/userinfo") as response:
                return await response.json()
